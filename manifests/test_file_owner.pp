...
#) inherits tdc {
  ...
  $file.each | $f, $fff | {
    concat::fragment { "${fff} ${f}":
      target  => "${tdc::checkrootdir}/${tdc::checkconfigdir}/tdc_${title}-file-owner.cfg",
      content => "command[check_tdc_${title}-${f}-${lfqdn}-file-owner]=${nagioscheck} ${fff['file']} ${fff['owner']}\n",
      notify  => Service[$tdc::nrpeservice],
    }
    generate ('/bin/bash', '-c',
    "${tdc::generator} ${nagiosout} service yes check_tdc_${title}-${f}-${lfqdn}-file-owner")
    generate ('/bin/bash', '-c',
    "${tdc::generator} ${nagiosout} hostgroup yes check_tdc_${title}-${f}-${lfqdn}-file-owner ${lfqdn}")
  }
