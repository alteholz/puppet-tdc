...
class tdc::test_file (
  Array   $file        = [],
  String  $nagiosout   = "${::tdc::nagiosdir}/tdc-${::fqdn}-${::tdc::caller_module_name}-file",
  ...
  $file.each | $f, $fff | {
    concat::fragment { "${fff} ${f}":
      target  => "${::tdc::checkrootdir}/${::tdc::checkconfigdir}/tdc_${::tdc::caller_module_name}-file.cfg",
      content => "command[check_tdc_${::tdc::caller_module_name}-${f}-${::fqdn}-file]=${nagioscheck} ${fff}\n",
      notify  => Service[$::tdc::nrpeservice],
    }
    generate ('/bin/bash', '-c',
              "${::tdc::generator} ${nagiosout} service yes check_tdc_${::tdc::caller_module_name}-${f}-${::fqdn}-file")
    generate ('/bin/bash', '-c',
              "${::tdc::generator} ${nagiosout} hostgroup yes check_tdc_${::tdc::caller_module_name}-${f}-${::fqdn}-file ${::fqdn}")
  }
...
# generate ("/bin/bash", "-c", "${::tdc::generator} ${::tdc::nagiosdir}/tdc-$fqdn-${::tdc::caller_module_name}-file host no $fqdn")
}
