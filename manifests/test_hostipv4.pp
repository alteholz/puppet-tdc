...
define tdc::test_hostipv4 (
  Array   $host         = [],
  String  $nagiosout    = "${::tdc::nagiosdir}/tdc-${::fqdn}-${title}-hostipv4",
...
#) inherits tdc {
  ...
  $host.each | $f, $fff | {
    concat::fragment { "${fff} ${f}":
      target  => "${::tdc::checkrootdir}/${::tdc::checkconfigdir}/tdc_${title}-hostipv4.cfg",
      content => "command[check_tdc_${title}-${f}-${::fqdn}-hostipv4]=${nagioscheck} -H ${fff} -4 -w ${pingwarning} -c ${pingcritical}\n",
      notify  => Service[$::tdc::nrpeservice],
    }
    generate ('/bin/bash', '-c',
              "${::tdc::generator} ${nagiosout} service yes check_tdc_${title}-${f}-${::fqdn}-hostipv4")
    generate ('/bin/bash', '-c',
              "${::tdc::generator} ${nagiosout} hostgroup yes check_tdc_${title}-${f}-${::fqdn}-hostipv4 ${::fqdn}")
  }
