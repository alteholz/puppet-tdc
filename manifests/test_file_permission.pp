...
#) inherits tdc {
  ...
  $file.each | $f, $fff | {
    concat::fragment { "${fff} ${f}":
      target  => "${tdc::checkrootdir}/${tdc::checkconfigdir}/tdc_${title}-file-permission.cfg",
      content => "command[check_tdc_${title}-${f}-${facts['networking']['fqdn']}-file-permission]=${nagioscheck}
        ${fff['file']} ${fff['permission']}\n",
      notify  => Service[$tdc::nrpeservice],
    }
  generate ('/bin/bash', '-c', "${tdc::generator} ${nagiosout} service yes
    check_tdc_${title}-${f}-${facts['networking']['fqdn']}-file-permission")
  generate ('/bin/bash', '-c', "${tdc::generator} ${nagiosout} hostgroup yes
    check_tdc_${title}-${f}-${facts['networking']['fqdn']}-file-permission ${facts['networking']['fqdn']}")
  }
